...
if __name__ == "__main__":
    ...
    argumentParser = ArgumentParser(
        ...
        ' workflow output. Two pairs are printed, one guaranteed to have no'
        ' whitespace.'
    )
    argumentParser.add_argument(
        "--no-space-suffix", dest='spacelessSuffix', type=str, metavar="SUFFIX"
        , default="Spaceless", help=
        'Suffix to add to the key in the pair with no whitespace.'
        ' Default is "Spaceless".')
    argumentParser.add_argument(
    ...
    value = get_ini_value(arguments.path, arguments.section, arguments.key)
    key = arguments.key if arguments.alias is None else arguments.alias
    for printKey, printValue in (
        (key, value),
        (key + arguments.spacelessSuffix, '_'.join(value.split()))
    ):
        print('='.join((printKey, printValue)))
